...
test-api:
	...
	docker run -d --rm --volume `pwd`:/opt/calc --network calc-test-api --env PYTHONPATH=/opt/calc --name apiserver --env FLASK_APP=app/api.py -p 5200:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
...
test-e2e:
	...
	docker run -d --rm --volume `pwd`:/opt/calc --network calc-test-e2e --env PYTHONPATH=/opt/calc --name apiserver --env FLASK_APP=app/api.py -p 5100:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
